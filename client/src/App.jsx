...
const PROXY_URL = "http://192.168.1.10:8001";
    ...
    <div className="min-h-screen bg-base-200 text-base-content transition-colors duration-300">
      ...
      <div className="navbar glass-effect sticky top-0 z-50 px-6 py-4 border-b border-base-content/5 shadow-sm">
        <div className="flex-1 flex items-center gap-3 group">
          <div className="p-2 bg-primary rounded-xl text-primary-content shadow-lg shadow-primary/20 group-hover:rotate-12 transition-transform duration-300">
            <Monitor className="w-5 h-5" />
          </div>
          <div>
            <h1 className="text-lg font-black tracking-tight leading-none uppercase">DES</h1>
            <p className="text-[10px] font-bold opacity-40 tracking-widest leading-none uppercase mt-1">Examination Client</p>
          </div>
        </div>
        <div className="flex-none gap-2">
          <button 
            onClick={toggleTheme} 
            className="btn btn-ghost btn-circle rounded-xl hover:bg-base-300"
          >
      ...
      <main className="container mx-auto px-6 py-12 flex flex-col items-center justify-center min-h-[calc(100vh-80px)]">
        <div className="w-full max-w-2xl animate-in fade-in slide-in-from-bottom-8 duration-700">
          {!registered ? (
            <div className="card bg-base-100 shadow-2xl border border-base-300/50 rounded-[2.5rem] p-10 text-center space-y-8">
              <div className="flex justify-center">
                <div className="p-6 bg-error/10 text-error rounded-3xl animate-pulse-subtle">
                  <AlertCircle className="w-16 h-16" />
                </div>
              </div>
              <div className="space-y-3">
                <h2 className="text-4xl font-black tracking-tight">Client Not Registered</h2>
            ...
            <div className="card bg-base-100 shadow-2xl border border-base-300/50 rounded-[2.5rem] overflow-hidden">
              <div className="bg-success/5 p-10 text-center space-y-8 border-b border-success/10">
                <div className="flex justify-center">
                  <div className="p-6 bg-success/10 text-success rounded-3xl">
                    <CheckCircle2 className="w-16 h-16" />
                  </div>
                </div>
                <div className="space-y-2">
                  <h2 className="text-4xl font-black tracking-tight text-success">Registered</h2>
                  <div className="badge badge-success badge-lg gap-2 py-4 px-6 rounded-xl font-bold uppercase tracking-widest text-xs">
              ...
              <div className="p-10 space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-base-200 rounded-3xl border border-base-300 space-y-2">
                    <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">Client ID</p>
                    <p className="font-mono text-sm break-all truncate">{clientDetails?.client_id}</p>
                  </div>
                  <div className="p-6 bg-base-200 rounded-3xl border border-base-300 space-y-2">
                    <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">Heartbeat Status</p>
                    <div className="flex items-center gap-2">
                      <Heart className={`w-4 h-4 ${heartbeatActive ? "text-error fill-error animate-ping" : "opacity-20"}`} />
