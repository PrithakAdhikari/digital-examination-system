...
import { useTeacherExamSummary } from "../../../hooks/useTeacherQueries.js";

const COLORS = ["#36d399", "#fbbd23", "#9ca3af"];

export default function TeacherExaminationOverviewCard() {
  const { data, isLoading, isError, error, refetch } = useTeacherExamSummary();
  ...
  const total = data?.total ?? finished + ongoing + upcoming;

  const chartData = [
    { name: "Finished", value: finished, fill: COLORS[0] },
    { name: "Ongoing", value: ongoing, fill: COLORS[1] },
    { name: "Upcoming", value: upcoming, fill: COLORS[2] },
    ...
    <div className="glass-card card h-full shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
      <div className="card-body p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold tracking-tight">Examination Overview</h2>
        ...
        {chartData.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 opacity-40">
            <p className="text-sm font-medium">No assigned examination data</p>
          </div>
        ) : (
            ...
            <div className="h-44 w-full relative">
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-center">
                  ...
                  <p className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest">Assigned</p>
                </div>
              </div>
